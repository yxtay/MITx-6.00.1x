...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ## YOUR CODE HERE ####
        a_parent_path = self.getParentPath(a)
        b_parent_path = self.getParentPath(b)
        a_length = len(a_parent_path)
        b_length = len(b_parent_path)
        
        for i in range(min(a_length, b_length)):
            # find location of first different parents
            if a_parent_path[i] != b_parent_path[i]:
                break
        # if no break, they are either the same node
        # or one is a direct decendent of another
        else:
            i += 1
            
        cousin_type = min(a_length, b_length) - (i + 1)
        degree_removed = abs(a_length - b_length)
        
        return (cousin_type, degree_removed)
        
    def getParentPath(self, a):
        """
        Returns list of parents beginning from founder to Member(a), inclusive
        """
        # list to contain path from founder to Member(a)        
        parent_path = []        
        # loop to insert parent and get next parent up the tree
        # beginning with Member(a) itself
        a_node = self.names_to_nodes[a]
        while a_node:
            parent_path.insert(0, str(a_node))            
